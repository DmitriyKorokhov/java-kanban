...
public class HandlerSubtask extends HandlerAbstractTask {
    public HandlerSubtask(TaskManager taskManager, String requestPath, TypesOfTasks taskType) {
        super(taskManager);
    ...
    protected void getTask(HttpExchange httpExchange, String path, TypesOfTasks taskType, String query) {
        if (query != null) {
            ...
            Task task = taskManager.getTaskTable().get(taskId);
            ...
            } else if (query == null && path.equals(requestPath)) {
                String jsonString = gson.toJson(taskManager.getListSubtasks());
                createResponse(httpExchange, gson.toJson(jsonString), 200);
            } else {
                createResponse(httpExchange,"Не найден", 404);
            }
        } else {
            createResponse(httpExchange, "Не найден", 404);
        }
    ...
    protected void postTask(HttpExchange httpExchange, TypesOfTasks taskType) {
        try {
            ...
            Task task = gson.fromJson(bodyRequest, Subtask.class);
    ...
    protected void deleteTask(HttpExchange httpExchange, String path, TypesOfTasks taskType, String query) {
        try {
            if (query == null && path.equals(requestPath)) {
                ...
                createResponse(httpExchange, "Все задачи удалены", 200);
            } else if (query != null) {
                int taskId = Integer.parseInt(query.substring(3));
                taskManager.clearByIdSubtasks(taskId);
